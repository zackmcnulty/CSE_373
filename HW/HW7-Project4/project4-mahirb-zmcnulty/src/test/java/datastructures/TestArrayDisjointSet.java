...
public class TestArrayDisjointSet extends BaseTest {
    ...
    @Test(timeout = SECOND)
    public void testUpdatesSizeCorrectlySmall() {
        IDisjointSet<Integer> forest = new ArrayDisjointSet<>();
        for (int i = 0; i < 10; i++) {
            forest.makeSet(i);
        }
        forest.union(0, 1);
        assertEquals(forest.findSet(0), forest.findSet(1));
        assertEquals(forest.findSet(1), 0);
        forest.union(1, 2);
        assertEquals(forest.findSet(1), forest.findSet(2));
        assertEquals(forest.findSet(2), 0);
    }
    
    @Test(timeout = 5 *SECOND)
    public void testUpdatesSizeCorrectlyMany() {
        IDisjointSet<Integer> forest = new ArrayDisjointSet<>();
        ...
        for (int i = 1; i < 200; i++) {
            assertEquals(0, forest.findSet(i));
        }
        for (int i = 1000; i > 801; i--) {
            forest.union(i, i-1);
        }
        for (int i = 999; i > 801; i--) {
            assertEquals(1000, forest.findSet(i));
        }
        forest.union(100, 850);
        assertEquals(forest.findSet(900), forest.findSet(15));
        assertEquals(forest.findSet(900), 0);
    }
    
    @Test(timeout = 5*SECOND)
    public void testLargeLinearForest() {
        IDisjointSet<Integer> forest = new ArrayDisjointSet<>();
        int cap = 1000000;
        for (int i = 0; i < cap; i++) {
            forest.makeSet(i);
