...
public class Searcher {
    ...
    public static <T extends Comparable<T>> IList<T> topKSort(int k, IList<T> input) {
        ...
        IPriorityQueue<T> heap = new ArrayHeap<T>(k + 2); // avoids having to resize heap
        Iterator<T> iter = input.iterator();
        
        // The heap keeps track of the k largest elements we have encountered thus far.
        for (int i = 0; i < k; i++) {
        ...
        IList<T> maxElements = new DoubleLinkedList<T>(); 
        while (!heap.isEmpty()) {
            maxElements.add(heap.removeMin());
        }
        return maxElements;
    }
}
